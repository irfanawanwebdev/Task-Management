...
export default function WorkloadPage() {
  ...
  const healthy    = rows.filter(r => r.workload_status === 'Healthy').length
  const busy       = rows.filter(r => r.workload_status === 'Busy').length
  const overloaded = rows.filter(r => r.workload_status === 'Overloaded').length
  const totalActive = rows.reduce((sum, r) => sum + r.active_tasks, 0)
  const totalOverdue = rows.reduce((sum, r) => sum + r.overdue_tasks, 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
        <SummaryCard label="Total Members" value={rows.length} />
